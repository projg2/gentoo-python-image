...
RUN echo '*/* ~amd64' >> /etc/portage/package.accept_keywords \
 ...
 && emerge -1vnt --jobs dev-python/tox app-arch/lzip dev-vcs/git dev-python/pypy{,3}-exe-bin \
 && emerge -1v --jobs --nodeps dev-lang/python:{2.7,3.4,3.5,3.6,3.7,3.8} dev-python/pypy{,3} \
 && rm -r /var/db/repos/*
